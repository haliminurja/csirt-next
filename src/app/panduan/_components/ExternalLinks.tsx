...
import { getPdfRoute, type PdfDocSlug } from '@/lib/pdf-docs';
...
export default function ExternalLinks() {
  const handlePdfClick = (slug: PdfDocSlug) => {
    window.open(getPdfRoute(slug), '_blank', 'noopener,noreferrer');
  };
  ...
  const externalDocs: ExternalDoc[] = [
    ...
    },
    {
      ...
      slug: 'penanganan-web-defacement',
    ...
    },
    {
      ...
      slug: 'keamanan-siber-bisnis-kecil',
      bgClass: 'bg-blue-50',
      textClass: 'text-blue-600',
      hoverClass: 'group-hover:bg-blue-600',
    },
  ...
  return (
    <section className="relative border-t border-slate-200 bg-slate-50 py-16 sm:py-24 dark:border-slate-800 dark:bg-slate-950">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        
        <div className="flex flex-col items-center justify-center text-center mb-16">
          <span className="mb-3 rounded-full border border-blue-200 bg-blue-100/50 px-4 py-1.5 text-sm font-bold tracking-wider text-blue-600 uppercase dark:border-blue-800 dark:bg-blue-900/30 dark:text-blue-300">Referensi Lanjutan</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {externalDocs.map((doc) => (
            <div 
              key={doc.slug}
              onClick={() => handlePdfClick(doc.slug)}
              className="group relative z-10 flex h-full cursor-pointer flex-col items-start overflow-hidden rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition-all duration-300 hover:-translate-y-1.5 hover:shadow-xl dark:border-slate-800 dark:bg-slate-900"
            >
              ...
              <div className={`mb-5 flex h-12 w-12 items-center justify-center rounded-xl border border-slate-100 shadow-sm transition-colors duration-300 group-hover:text-white dark:border-slate-700 ${doc.bgClass} ${doc.textClass} ${doc.hoverClass}`}>
              ...
              <div className="mb-2">
                <span className="mb-1 block text-xs font-bold tracking-wider text-slate-400 uppercase dark:text-slate-500">{doc.sender}</span>
                <h3 className="line-clamp-2 text-base font-bold leading-snug text-slate-800 transition-colors group-hover:text-blue-600 dark:text-slate-100 dark:group-hover:text-blue-300">{doc.title}</h3>
              </div>
              
              <div className="mt-auto pt-5 w-full flex items-center justify-between text-sm">
                <span className="flex items-center font-medium text-slate-500 group-hover:text-slate-700 dark:text-slate-400 dark:group-hover:text-slate-200">
                  <svg className="mr-1.5 h-4 w-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" /></svg>
                  Format PDF
                </span>
                <span className="flex h-8 w-8 items-center justify-center rounded-full border border-slate-200 bg-slate-50 text-slate-400 transition-all group-hover:border-blue-600 group-hover:bg-blue-600 group-hover:text-white dark:border-slate-700 dark:bg-slate-800 dark:text-slate-500 dark:group-hover:border-blue-600 dark:group-hover:bg-blue-600 dark:group-hover:text-white">
